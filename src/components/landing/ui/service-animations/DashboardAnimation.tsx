...
export function DashboardAnimation({ locale = "en" }: { locale?: Locale }) {
    ...
    return (
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden bg-[#0a0a0a]">
            {/* 
              We use a scaling container for responsiveness. It maxes out at a desktop width 
              and naturally scales down via CSS transform on mobile to avoid squashing.
            */}
            <div className="w-full h-full max-h-[400px] max-w-[600px] flex flex-col justify-between p-3 sm:p-6 lg:p-8 transform scale-[0.85] sm:scale-100 origin-center">

                {/* Top KPIs Row - Users, Revenue, Costs */}
                <div className="w-full flex gap-3 sm:gap-4">
                    {kpis.map((kpi, i) => (
                        <motion.div
                            key={i}
                            className="flex-1 bg-white/5 border border-white/10 rounded-xl p-2 sm:p-3 lg:p-4 shadow-lg flex flex-col justify-between"
                        ...
                        >
                            ...
                            <div className="flex items-end justify-between">
                                <span className="text-sm sm:text-lg lg:text-xl font-black text-white">{kpi.value}</span>
                ...
                <div className="w-full flex gap-3 sm:gap-4 my-3 sm:my-4 flex-1 min-h-[120px] sm:min-h-[140px]">
                    ...
                    <div className="flex-[3] bg-[#151515] border border-white/10 rounded-xl p-3 sm:p-4 lg:p-5 flex flex-col shadow-2xl overflow-hidden relative">
                        ...
                        <div className="relative flex-1 flex items-end gap-2 sm:gap-3 lg:gap-4 pb-1 z-0 w-full">
                            ...
                            <svg className="absolute inset-0 h-full w-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
                                <motion.polyline
                                    points={LINE_POINTS}
                                    fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
                    ...
                    <div className="flex-[2] bg-white/5 border border-white/10 rounded-xl p-2 sm:p-3 lg:p-4 flex flex-col gap-2 sm:gap-3 shadow-inner overflow-hidden min-w-[35%]">
                        <span className="text-[9px] lg:text-xs font-black uppercase text-[#00BCD4] tracking-wider mb-0 sm:mb-1 flex items-center gap-1 sm:gap-2">
                            <span className="text-sm sm:text-base">🧠</span> Copilot
                        </span>
                        <div className="flex-1 bg-[#0a0a0a]/50 rounded-lg p-2 sm:p-3 flex flex-col gap-2 relative border border-white/5">
                            <motion.div
                            ...
                            >
                                <AnimatePresence mode="popLayout">
                                    <motion.div
                                        ...
                                        className="bg-white/10 p-2 sm:p-2.5 lg:p-3 rounded-md text-[8px] sm:text-[9px] lg:text-xs text-white/90 font-medium leading-tight border border-white/10"
                                    >
                                        {msgs[msgIdx]}
                                    </motion.div>
                                </AnimatePresence>
                ...
                <div className="w-full bg-[#151515] border border-white/10 rounded-xl px-3 py-2 sm:px-4 sm:py-3 lg:px-6 lg:py-4 flex justify-between items-center shadow-lg">
                    <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
                        ...
                        >⚙️</motion.div>
                        <span className="text-[9px] sm:text-[11px] lg:text-xs font-black uppercase tracking-widest text-white/70">
                            {locale === 'es' ? 'Motor de Operaciones' : 'Logistics Engine'}
                        </span>
                    </div>
                    <div className="flex -space-x-1 sm:-space-x-2 shadow-sm">
                        {["👩‍🔧", "👨‍🍳", "🧘‍♀️", "🚌"].map((emoji, i) => (
                            <div key={i} className="h-6 w-6 sm:h-7 sm:w-7 lg:h-8 lg:w-8 rounded-full bg-[#2a2a2a] border-2 border-[#1a1a1a] flex items-center justify-center text-[10px] sm:text-xs lg:text-sm z-10 shrink-0">
                                {emoji}
                            </div>
                        ))}
                        <div className="h-6 w-6 sm:h-7 sm:w-7 lg:h-8 lg:w-8 rounded-full bg-[#00BCD4] border-2 border-[#1a1a1a] flex items-center justify-center text-[9px] sm:text-[10px] lg:text-xs font-black z-0 shrink-0 text-white">
                            +8
                        </div>
                    </div>
                </div>
