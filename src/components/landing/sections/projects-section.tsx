      ...
      <div>
        {content.projects.map((project, i) => (
          <button
            type="button"
            key={i}
            className="group border-b-4 border-black last:border-b-0 grid lg:grid-cols-12 bg-white hover:bg-neutral-50 transition-colors cursor-pointer text-left w-full"
          ...
          >
              ...
              <Reveal>
                <div className="flex flex-wrap gap-2 mb-4 md:mb-6">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="bg-black text-white px-3 py-1 font-bold text-xs md:text-sm uppercase shadow-[3px_3px_0px_0px_var(--primary)]"
                    >
                      {tag}
                    </span>
                  ))}
                  <span className="bg-secondary text-secondary-foreground border-2 border-black px-3 py-1 font-bold text-xs md:text-sm uppercase">
                    {project.status}
                  </span>
                </div>
                <h3 className="text-3xl md:text-4xl lg:text-5xl font-black uppercase mb-4 md:mb-6 leading-tight group-hover:text-primary transition-colors">
                  {project.title}
                </h3>
                <p className="text-lg md:text-xl font-bold mb-6 md:mb-8 max-w-2xl">
                  {project.desc}
                </p>
                <div className="bg-muted border-l-8 border-black p-4 md:p-6">
            ...
            <div className="lg:col-span-5 h-[300px] md:h-[400px] lg:h-auto relative overflow-hidden order-1 lg:order-2 border-b-4 lg:border-b-0 border-black lg:border-l-0">
              <div className="absolute inset-0 bg-primary mix-blend-multiply opacity-0 group-hover:opacity-60 transition-opacity duration-300 z-10 pointer-events-none"></div>
              <Image
                src={project.image}
                alt={project.title}
                fill
